...
var text1 ='{"nameXAxis": "Zeit", "data": [{"pointid": 1, "starttime": 1454396400000, "endtime": 1454409000000, "comment": "Gestörter Bauablauf", "y": 3, "yOriginal":3, "color": "#e57373", "Status": 3, "hasChildren":false }, { "pointid": 2, "starttime": 1454412000000, "endtime": 1454414400000, "comment": "", "y": 3, "yOriginal":3, "Status": 3, "color": "#ff9800", "hasChildren":false }, { "pointid": 3, "starttime": 1454414400000, "endtime": 1454421298000, "comment": "", "y": 5, "yOriginal":5, "Status": 3, "color": "#3f51b5", "hasChildren":false },{ "pointname": "258F_01213a1_1_2", "pointid": 4, "starttime": 1454420000000, "endtime": 1454422301000, "comment": null, "y": 1, "yOriginal":1, "Status": 3, "color": "#2196f3","hasChildren":false },{ "pointid": 5, "starttime": 1454422374000, "endtime": 1454423442000, "comment": null, "y": 2, "yOriginal":2, "Status": 3, "color": "#81d4fa", "hasChildren":false } ]}';
...
//a set of test additional process data 
var extraData = [];
//load the base text data  
data.dataObject=JSON.parse(text1);
//load the Y axis line names  
data.axisTickTransform=JSON.parse(text2);

//create test additional process data 
function freshExtraData()
{
	extraData=[];

	for (var i=0; i< data.dataObject.data.length; i++)
	{//add activities for each timesheet
		var pid=data.dataObject.data[i].pointid;
		var cmnt=data.dataObject.data[i].comment;
...
//		var yO=data.dataObject.data[i].yOriginal;
		...
		extraData.push({"pointid": pid, "starttime": st, "endtime":st+(totlen/4) , "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2": 1, "detail":true , "maxY":0});
		extraData.push({"pointid": pid, "starttime": st+(3*(totlen/4)), "endtime":et , "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2": 1, "detail":true , "maxY":0});
//mix them up a bit to have different size (Y's) per process
		if(i!=1)
		{
			extraData.push({"pointid": pid, "starttime": st+ (2*(totlen/4)), "endtime": st+(3*(totlen/4)), "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2": 1, "detail":true , "maxY":0 });
			extraData.push({"pointid": pid, "starttime": st+ (totlen/4), "endtime": et- (totlen/3), "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2": 2, "detail":true , "maxY":0 });
			extraData.push({"pointid": pid, "starttime": st, "endtime": st+(totlen/4), "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2": 3, "detail":true , "maxY":0 });
			extraData.push({"pointid": pid, "starttime": st+ (2*(totlen/4)), "endtime": et, "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2": 3, "detail":true , "maxY":0 });
			if(i>2)
			{
				extraData.push({"pointid": pid, "starttime": st+ (totlen/4), "endtime": et- (totlen/3), "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2":4, "detail":true , "maxY":0 });
				extraData.push({"pointid": pid, "starttime": st, "endtime": et, "comment": cmnt, "y": yy, "yOriginal": yO, "color": olCol, "Status": 3, "y2":5, "detail":true , "maxY":0 });
			}
		}
	}
}//end freshExtraData()
...
//	var yPos=interval.data[0].yOriginal;
//	var yDraw=interval.data[0].y;

	var cpid=interval.data[0].clickedID;
	var dataToReturn = {data:[], maxY2:0};
	freshExtraData();

	//traverse extraData and filter irrelevant data...
	for (let k=0; k<extraData.length;k++) {
//		if(extraData[k].y==yPos && extraData[k].endtime<=enTime && extraData[k].starttime>=stTime)
		if(extraData[k].pointid===cpid)
		{
